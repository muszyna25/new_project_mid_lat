...
def get_largest_blob_size_in_region(reg_img, x):
    ...
    #    for y in range(0, reg_img.shape[1]):
    #        val = reg_img[x][y]
    #        if val != 0:
    #            blob_s += 1
...
def extract_regions(f_var, d_reg_loc, D):
    ...
    for dk in D.keys():
        ...
        #c_ind = 1100
        #plot_imgs(img[c_ind], reg_img[c_ind])
        ...
        labels = []
        for x in range(0, reg_img.shape[0]):
            bs = get_largest_blob_size_in_region(reg_img[x], x)
...
if __name__ == "__main__":
    ...
    fn_list = sys.argv[1]
    frac_area = float(sys.argv[2])
    out_path = sys.argv[3]
    
    print(sys.argv[0], fn_list, frac_area, out_path)
    ...
    d_reg_bin_masks = {'NC': {'X': [], 'Y': []}, 'NP': {'X': [], 'Y': []}, 'NA': {'X': [], 'Y': []}, 
         ...
         #out_path #='/global/cscratch1/sd/muszyng/ethz_data/project_atmo_block_datasets/generated_datasets/corrected_labels/test/all_corrected_labels' 
