...
VERSION="soft"      # Version of labeling: https://docs.google.com/document/d/1hGE5t1Q6V9UrZ0gSwaetktFtL7j3BSyMvs85kGAI0Hg/edit?usp=sharing

VAR='u'             # Variable in ERA-interim data.
...
######################################## 6.py ######################################## # Split HDF5 file data into three files: train.h5, val.h5, test.h5.
# nothing --

######################################## 7.py ######################################## # Normalize data in each of three files.
# nothing --

######################################## 8.py ######################################## # Split each of the three files into k subsets for data generator (streaming).
OUT_CHUNK_FILES_HDF5="${OUT_BIG_FILES_HDF5}/chunk_files"

mkdir -p ${OUT_CHUNK_FILES_HDF5}

######################################## MAIN ########################################
#If labeled data've been already extracted.
#if [ $FLAG_EXTRACT_LABELS -eq "1" ]; then 
#python ${FILES[0]} ${L_BIN_MASKS} ${OUT_LABELS_PATH} && 
#else 
#    echo "Skipped ${FILES[0]}" 
#fi 

# If raw data regions've been already extracted.
#if [ $FLAG_EXTRACT_DATA -eq "1" ]; then 
#python ${FILES[1]} ${L_ERA_FILES} ${VAR} ${OUT_RAW_DATA_PATH} && 
#else 
#    echo "Skipped ${FILES[1]}" 
#fi 
