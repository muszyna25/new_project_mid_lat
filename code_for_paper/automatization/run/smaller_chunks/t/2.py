...
def extract_regions(f_var, d_reg_loc, D):
    ...
    for dk in D.keys():
        ...
        if dk == 'NA' or dk == 'SA':
            ...
            #reg_img = np.concatenate((reg_img_E, reg_img_W), axis=2)
        else:
...
if __name__ == "__main__":
    ...
    for i in range(len(df_list)):
        ...
        if (var in vars_list):
            print('Extract one variable: ', var)
            f_var_tmp = read_netcdf_file(dir_path + f, var)
            print('Check shape: ', f_var_tmp.shape)
            #X_ = np.swapaxes(f_var_tmp,1,2)
            #f_var = np.swapaxes(X_,2,3)
            #extract_regions(f_var, d_reg_loc, d_reg_bin_masks)
            extract_regions(f_var_tmp, d_reg_loc, d_reg_bin_masks)
            save_to_hdf5_format(d_reg_bin_masks, f, out_path)
        else:
            print('Extract all variables')
            X_all_vars = np.array([]) 
            for v in vars_list:
                print(v)
                X = read_netcdf_file(dir_path + f, v)
                print(X.shape)
                #X_ = np.swapaxes(f_var_tmp,1,2)
                #X = np.swapaxes(X_,2,3)
                #print(X.shape)
                if v == 'u':
                    X_all_vars = X
                else:
                    #X_all_vars = np.concatenate((X_all_vars, X), axis=3)
                    X_all_vars = np.concatenate((X_all_vars, X), axis=1)
            print('Check shape X_all_vars: ', X_all_vars.shape)

            extract_regions(X_all_vars, d_reg_loc, d_reg_bin_masks)
            save_to_hdf5_format(d_reg_bin_masks, f, out_path)
