...
path_labels = '../compare_regression_models/regressor_predict/avgs/'
#path_labels = '../classifier_predict/'

m_type = 'E'
f_name = path_labels + 'Model_' + m_type + '.h5'
...
for i in lfs:
    ...
    for a in A:
        ...
        REG_DATA[reg]['Y_t'].append(Y_t[counter])
        REG_DATA[reg]['Y_p'].append(Y_p[counter])

        TIME_DATA[year]['Y_t'].append(Y_t[counter])
        TIME_DATA[year]['Y_p'].append(Y_p[counter])
        counter += 1

    #print(TIME_DATA['1994'])
    #print(REG_DATA['NC'])

save_to_hdf5_format(REG_DATA, 'loc_reg_model_' + m_type, out_path)
save_to_hdf5_format(TIME_DATA, 'loc_time_model_' + m_type, out_path)
