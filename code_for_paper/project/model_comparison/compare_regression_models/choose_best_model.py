...
for a in arch_names:
    ...
    for cvr in range(0,cv_rounds):
        ...
        checkpoint_dir = os.path.join(os.environ['SCRATCH'],'cnn_model_runs/regression/%s/%s_%i/' % (a,a,cvr))
        ...
        if dirs != []: # This condition is not needed and can be removed when I executed all architectures.
            for d in dirs:
                ...
                for f in l_trains_sorted:
                    ...
                    print('empty: ', data)
                    if data != []:
                        last_train_acc = data[len(data)-1]['CCC']
                        #last_train_acc = data[len(data)-1]['acc']
                        #last_val_acc = data[len(data)-1]['val_acc']
                        last_val_acc = data[len(data)-1]['val_CCC']
