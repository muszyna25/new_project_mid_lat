...
def CC(y_true, y_pred):
    ...
    top=K.sum((n_y_true[:]-K.mean(n_y_true[:]))*(n_y_pred[:]-K.mean(n_y_pred[:])))
    bottom=K.sqrt(K.sum(K.pow((n_y_true[:]-K.mean(n_y_true[:])),2))*K.sum(K.pow(n_y_pred[:]-K.mean(n_y_pred[:]),2)))

    result=top/bottom

    print('RESULT: ', top.shape, bottom.shape, result.shape)
    '''
...
def CCC(y_true, y_pred):
    ...
    #s_xy = K.mean(K.sum((y_true - K.mean(y_true, axis=0)) * (y_pred - K.mean(y_pred, axis=0)), axis=0),axis=0)
    ...
    #tensorflow.keras.backend.print_tensor(s_xy,message='covariance')
    #tensorflow.python.keras
    #tmp1 = np.random.randint(0,10, (100,))
    #print('AAA'); print(tmp1)
    #tensorflow.print("tensors:",tmp)
    #tensorflow.print("tensors:",s_xy, output_stream=sys.stderr)
    #print(s_xy.eval())
    #v = s_x_sq + s_y_sq  
    #tensorflow.keras.backend.print_tensor(v,message='variance')
    #m = (x_m-y_m)**2
    #tensorflow.keras.backend.print_tensor(m,message='mean')

    # condordance correlation coefficient
    ccc = (2.0*s_xy) / (s_x_sq + s_y_sq + (x_m-y_m)**2)
    #ccc = (s_x_sq + s_y_sq + (x_m-y_m)**2)
    #ccc = (s_x_sq + s_y_sq)
    #ccc = (x_m-y_m)**2
    #t_obj = (s_xy)
    #np_arr = tensorflow.make_ndarray(t_obj)
    #np_arr = tensorflow.make_ndarray(y_pred)
    #a_prt = tensorflow.make_tensor_proto(y_pred[0])
    #t_obj = K.variable(y_pred)
    #L_CCC.append(a_prt)
    #ccc = (2.0*s_xy) / ((s_x_sq + s_y_sq + (x_m-y_m)**2) + K.epsilon())

    #ccc = K.divide(2.0*s_xy, s_x_sq + s_y_sq + (x_m-y_m)**2)
...
##### MODEL ARCHITECTURE #####    
    ...
    #model.compile(optimizer=opt, loss='mean_squared_error', metrics=['mse', cc0_for_channel(0), cc1_for_channel(1), cc2_for_channel(2)])
    model.compile(optimizer=opt, loss='mean_squared_error', metrics=['mse', CCC, cc0_for_channel(0), cc1_for_channel(1), cc2_for_channel(2), ccc_for_channel(1)])
...
def train_model(model,
    ...
    #            generator=inpGenD['val'], 
    #            #steps=327,
    #            #validation_data=inpGenD['val'], 
    #            callbacks=rpv_callbacks, #shuffle=True, 
    #            verbose=1)

    #with h5py.File(data_dir_path + '/' + '2_val_0_.h5' , 'r') as h5f:
    #    X=h5f['X'][:]

    #y = model.predict(X)
    #with open('prediction.csv', 'ab') as f:
    #    print(y)
    #    np.savetxt(f, y)

    #print(L_CCC)
    #tensorflow.print("tensors:",L_CCC[0], output_stream=sys.stderr)
