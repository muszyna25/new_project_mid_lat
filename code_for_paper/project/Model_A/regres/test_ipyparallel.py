...
if __name__ == "__main__":
    ...
    n_fc_layers = 3
    dense_sizes = np.array([256, 128, 64])
    #dense_sizes = np.array([128, 64, 32])
    ...
    dropout = np.random.uniform(0.3, 0.5, size=n_hpo_trials)
    ...
    checkpoint_dir = os.path.join(os.environ['SCRATCH'],'cnn_model_runs/regression/%s/%s/%i' % ('ABC', 'ABC', 100000000))
    os.makedirs(checkpoint_dir, exist_ok=True)
    print(checkpoint_dir)
    checkpoint_file = os.path.join(checkpoint_dir, 'model_%i.h5' % ihp)

    build_and_train(input_dir, input_size,
            conv_sizes=conv_sizes[ihp], fc_sizes=fc_sizes[ihp], dropout=dropout[ihp], optimizer=optimizer[ihp], lr=lr[ihp],
            batch_size=batch_sizes[ihp], n_epochs=n_epochs, repeats_conv=[], recep_field=(3,3), 
            checkpoint_dir=checkpoint_dir, checkpoint_file=checkpoint_file, 
            l_fnames_train=l_fnames_train, l_fnames_val=l_fnames_val, ihp=ihp, verbose=0)
    ...
    checkpoint_dir = os.path.join(os.environ['SCRATCH'],'cnn_model_runs/regression/%s/%s/%i' % (model_name[:-2], model_name, job_id))
    os.makedirs(checkpoint_dir, exist_ok=True)
    print(checkpoint_dir)
